...
def extract_fields_from_image_region(
    ...
    #    - 其中內容需包含年,月,日
    field_instructions = {
        "備註": """- 備註：發票明細品名/數量/單價/金額以外的補充說明文字。
        【讀取範圍】
        - 若「備註」是表格中的獨立欄位，請依照每一筆明細列，讀取該列備註欄中的完整內容。
        - 每一列備註欄中的手寫數字、編號或文字，必須視為同一筆完整內容。
        - 不同明細列之間才使用「；」分隔。
        - 若備註是表格外以「備註：」開頭的自由文字，請讀取冒號後方內容。

        【數字完整性規則】
        - 同一列中的連續手寫數字必須合併為一個完整字串。
        - 即使數字之間有空隙、筆畫斷裂、書寫傾斜或部分重疊，也不可自行拆成多段。
        - 不可在同一列數字中間加入「；」、空格、逗號或其他分隔符號。
        - 「；」只能用來分隔不同明細列的備註內容。
        - 請依照圖片實際筆畫逐字辨識，不可自行補數字、刪除數字或重新排列。

        【手寫字形誤判提醒】
        - 手寫的「2」可能看起來像「>」「＞」「乙」。
        - 手寫的「7」可能看起來像「>」「丁」或斜線。
        - 手寫的「0」可能看起來像「O」「○」或未封閉的圓圈。
        - 遇到疑似符號的筆畫時，請結合前後數字與完整筆畫判斷，優先確認是否為數字。
        - 只有在圖片中確實存在符號時，才可輸出符號，不可將模糊數字直接輸出成符號。

        【總備註排除規則】
        - 若文字以「總備註：」開頭，請忽略該整段內容，不要輸出。
        - 此排除規則只適用於「總備註：」開頭的文字，不可因此忽略表格備註欄中的數字或編號。
        ...
        【輸出範例】
        - 圖片中第一列為「6700203988」，第二列為「6700204351」：
        輸出："6700203988；6700204351"

        - 錯誤輸出：
        "6700；203988；6700；204351"

        - 不可因同一列中間字距較大，就將完整編號拆開。
